...
class MexcExecutor:
    ...
    async def execute(
        ...
        configured_risk_percent = float(getattr(self.settings, "max_risk_per_trade", 1.0) or 1.0)
        # Older deployments used 0.01 to mean 1%; normalize that legacy form.
        if 0 < configured_risk_percent < 0.10:
            configured_risk_percent *= 100.0
        risk_percent = min(max(configured_risk_percent, 0.0), MAX_AUTO_TRADE_RISK_PERCENT)
