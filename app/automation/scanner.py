    ...
    async def _scan_one(self, symbol: str) -> dict[str, int]:
        async with self._semaphore:
            try:
                ...
                raw_5m = await self.client.get_klines(symbol, MEXC_INTERVALS["5M"], self.settings.candle_limit)

                closed_4h = closed_candle_rows(raw_4h, "4h")
                closed_1h = closed_candle_rows(raw_1h, "1h")
                closed_15m = closed_candle_rows(raw_15m, "15m")
                closed_5m = closed_candle_rows(raw_5m, "5m")

                analysis = analyze_candles(symbol, closed_4h, closed_1h, closed_15m)
                analysis["mexc_5m_rows"] = closed_5m

                # Final execution-price sanity check: analysis is based on closed
                # MEXC candles, but an actual MEXC trade must use a fresh
                # executable quote. Never silently turn a stale candle close into
                # a live order price.
                if analysis.get("setup") in {"LONG", "SHORT"}:
                    ticker = await self.client.get_ticker(symbol)
                    now_ms = int(datetime.now(tz=timezone.utc).timestamp() * 1000)
                    ticker_ts = int(ticker.get("timestamp", 0) or 0)
                    if ticker_ts <= 0 or now_ms - ticker_ts > int(self.settings.max_data_age_seconds * 1000):
                        LOGGER.info("Rejecting stale MEXC ticker for %s", symbol)
                    ...
                    if min(last_price, bid, ask) <= 0 or ask < bid:
                        LOGGER.info("Rejecting invalid MEXC quote for %s", symbol)
                        return {"valid": 0, "sent": 0, "errors": 0}
                    spread_pct = (ask - bid) / ((ask + bid) / 2.0)
                    if spread_pct > self.settings.max_mexc_spread_pct:
                        LOGGER.info("Rejecting wide MEXC spread for %s: %.5f", symbol, spread_pct)
                        return {"valid": 0, "sent": 0, "errors": 0}

                    index_price = float(ticker.get("indexPrice", 0) or 0)
                    fair_price = float(ticker.get("fairPrice", 0) or 0)
                    if index_price <= 0 or fair_price <= 0:
                        LOGGER.info("Rejecting missing MEXC index/fair price for %s", symbol)
                        return {"valid": 0, "sent": 0, "errors": 0}
                    index_dislocation = abs(last_price - index_price) / index_price
                    fair_dislocation = abs(last_price - fair_price) / fair_price
                    if max(index_dislocation, fair_dislocation) > self.settings.max_index_dislocation_pct:
                        LOGGER.info("Rejecting MEXC price dislocation for %s: index=%.5f fair=%.5f", symbol, index_dislocation, fair_dislocation)
                        return {"valid": 0, "sent": 0, "errors": 0}

                    depth = await self.client.get_depth(symbol, self.settings.orderbook_levels)
                    bids = depth.get("bids") or []
                    asks = depth.get("asks") or []
                    if not bids or not asks:
                        LOGGER.info("Rejecting missing MEXC orderbook for %s", symbol)
                        return {"valid": 0, "sent": 0, "errors": 0}

                    funding = await self.client.get_funding_rate(symbol)
                    deals = await self.client.get_deals(symbol, self.settings.trade_flow_limit)
                    buy_volume = sum(float(x.get("v", 0) or 0) for x in deals if int(x.get("T", 0) or 0) == 1)
                    sell_volume = sum(float(x.get("v", 0) or 0) for x in deals if int(x.get("T", 0) or 0) == 2)
                    total_flow = buy_volume + sell_volume
                    delta_proxy = (buy_volume - sell_volume) / total_flow if total_flow > 0 else 0.0

                    ticker["spread_pct"] = spread_pct
                    ticker["index_dislocation_pct"] = index_dislocation
                    ticker["fair_dislocation_pct"] = fair_dislocation
                    ...
                    if drift > max(0.0, self.settings.max_entry_drift_pct):
                        LOGGER.info(
                            "Rejecting stale MEXC entry for %s: planned=%s executable=%s drift=%.4f",
                            symbol, planned_entry, executable_price, drift,
                        )
                        return {"valid": 0, "sent": 0, "errors": 0}

                    # Reprice the prototype plan to the executable MEXC quote while
                    # preserving its current risk/target distances. Later strategy
                    # phases will replace these levels with structural SL/liquidity TP.
                    delta = executable_price - planned_entry
                    ...
                    analysis["rr"] = abs(analysis["tp2"] - analysis["entry"]) / abs(analysis["entry"] - analysis["stop_loss"])
                    analysis["mexc_executable_entry"] = executable_price
                    analysis["mexc_entry_drift_pct"] = drift
