...
def average_volume(candles: List[Dict], period: int = 20) -> float:
    if not candles:
        raise ValueError("No candles supplied")
    sample = candles[-period:]
    return sum(float(candle["volume"]) for candle in sample) / len(sample)


def volume_status(candles: List[Dict], period: int = 20) -> str:
    if len(candles) < period + 1:
        return "UNKNOWN"

    current_volume = float(candles[-1]["volume"])
    previous_average = average_volume(candles[:-1], period)
